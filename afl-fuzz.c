...
struct queue_entry {
  ...
  u8* fuzzed_branches_or_blocks;      /* @Ljie@ which branches or blocks have been done, [0, half) branch, [half,map_size) block */
...
/* @Ljie@ Things about blocks */
static int use_block_select = 0; /*Whether use block statistics to select*/
static int use_block_mutate = 0; /*Whether use block statistics to mutate*/

static int mutation_prior = 0; /*Whether prior the mutation rather than the normal AFL selection*/
...
static u32 target_index_plus_one = 0;      /* @Ljie@ non-zero branch index + 1 if fuzzing is being done with that branch constant*/
...
static int* get_lowest_hit_branch_ids(bool use_block){
  int * rare_branch_ids = ck_alloc(sizeof(int) * MAX_RARE_BRANCHES);
  int lowest_hob = INT_MAX;
  int ret_list_size = 0;

  u64 count_from = 0, count_to = HALF_MAP_SIZE;
  if(use_block) {
    count_from = HALF_MAP_SIZE;
    count_to = MAP_SIZE;
  }

  for (int i = count_from; (i < count_to) && (ret_list_size < MAX_RARE_BRANCHES - 1); i++){
  ...
  if (ret_list_size == 0){
    ...
    if (lowest_hob != INT_MAX) {
      ...
      return get_lowest_hit_branch_ids(use_block);
    }
  }
...
static u32 * is_rb_hit_mini(u8* trace_bits_mini, bool use_block){
  int * rarest_branches = get_lowest_hit_branch_ids(use_block);
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  q->fuzzed_branches_or_blocks = ck_alloc(MAP_SIZE >>3);
  // @End
...
EXP_ST void destroy_queue(void) {
  ...
  while (q) {
    ...
    ck_free(q->fuzzed_branches_or_blocks);
    ck_free(q);
    q = n;
...
static u32 count_branch_bits(u8* mem) {

  u32* ptr = (u32*)mem;
  u32  i   = (HALF_MAP_SIZE >> 2);
  u32  ret = 0;
...
static u32 count_block_bits(u8* mem) {
  u32 *ptr = mem + HALF_MAP_SIZE;
  u32 ret = 0;
  for (u32 *ptr_end = (u8 *)ptr + HALF_MAP_SIZE; ptr < ptr_end; ++ptr) {
...
static u32 count_branch_bytes(u8* mem) {
...
static u32 count_blcok_bytes(u8* mem) {

  u32* ptr = (u32*)(mem + HALF_MAP_SIZE);
  u32  i   = (HALF_MAP_SIZE >> 2);
  u32  ret = 0;
...
static u32 count_non_255_branch_bytes(u8* mem) {
...
static u32 count_non_255_block_bytes(u8* mem) {
...
static void update_bitmap_score(struct queue_entry* q) {
  ...
  //Here we find the top_rated for branch winner and block winner at the same time,
  //[0,HALF_MAP_SIZE) is branch winner 
  //[HALF_MAP_SIZE) is block winner
...
static void cull_queue(bool use_block) {

  struct queue_entry* q;

  //Here we calculated the favourite seeds, it is the selection part
  //Both use branch and block coverage may have conflicts, so here we would calculate it separately,
  //in other words, we calculate either by branch coverage or block coverage  
  static u8 temp_v[HALF_MAP_SIZE >> 3];
  u32 i;
  ...
  memset(temp_v, 255, HALF_MAP_SIZE >> 3);
  ...
  for (i = count_from; i < count_to; i++)
    if (top_rated[i] && (temp_v[i >> 3] & (1 << (i & 7)))) {

      u32 j = HALF_MAP_SIZE >> 3;
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  // u32 t1 = count_branch_bits(trace_bits);
  // u32 t2 = count_block_bits(trace_bits);
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (!dumb_mode && !stage_cur && !count_branch_bytes(trace_bits)) {
      fault = FAULT_NOINST;
  ...
  q->bitmap_size = count_branch_bytes(trace_bits);
  q->handicap    = handicap;
  q->cal_failed  = 0;
...
abort_calibration:
  ...
  if (var_detected) {
    ...
    var_byte_count = count_branch_bytes(var_bytes);
...
static void check_map_coverage(void) {
  ...
  if (count_branch_bytes(trace_bits) < 100) return;

  for (i = (1 << (HALF_MAP_SIZE_POW2 - 1)); i < HALF_MAP_SIZE; i++)
...
static void write_stats_file(double bitmap_cvg, double stability, double eps) {
  ...
  // for(int j = 0; j < i; j++) {
  //   if(is_min_branch[j]) {
  //     sum_cha2 = sum_cha2 + (hit_bits[j]-average)*(hit_bits[j]-average);
  //   }
  // }
  // double variance = (double)sum_cha2/(double)cnt;

  // t_bits = (MAP_SIZE << 3) - count_bits(virgin_bits);
  // t_bytes = count_non_255_bytes(virgin_bits); 

  u32 t_branch = (HALF_MAP_SIZE << 3) - count_branch_bits(virgin_bits);
  u32 t_block = (HALF_MAP_SIZE << 3) - count_block_bits(virgin_bits);
  ...
  fprintf(f, "start_time        : %llu\n"
             ...
             exec_tmout, t_branch,t_block, use_banner, orig_cmdline);
             /* ignore errors */
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  static u32 prev_qp, prev_pf, prev_pnf, prev_ce, prev_md, prev_branch, prev_block;
  ...
  if (prev_qp == queued_paths && prev_pf == pending_favored && 
      ...
      prev_branch == t_branch && prev_block == t_block) return;
  ...
  prev_md  = max_depth;
  prev_branch = t_branch;
  prev_block = t_block;

  
  /* Fields in the file:

     unix_time, ,t_branch, t_block,cycles_done, cur_path, paths_total, paths_not_fuzzed,
     favored_not_fuzzed, unique_crashes, unique_hangs, max_depth,
     execs_per_sec */

  fprintf(plot_file, 
          "%llu, %u, %u, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %u, %0.02f\n",
          get_cur_time() / 1000, prev_branch, prev_block, queue_cycle - 1, current_entry, queued_paths,
...
static void show_stats(void) {
  ...
  t_bytes = count_non_255_branch_bytes(virgin_bits);
  t_byte_ratio = ((double)t_bytes * 100) / HALF_MAP_SIZE;
  ...
  t_bits = (HALF_MAP_SIZE << 3) - count_branch_bits(virgin_bits);
...
#define SP20    SP10 SP10
  ...
  sprintf(tmp, "%0.02f%% / %0.02f%%", ((double)queue_cur->bitmap_size) * 
          100 / HALF_MAP_SIZE, t_byte_ratio);
...
static u8 trim_case(char** argv, struct queue_entry* q, u8* in_buf) {
  ...
  static u8 tmp[64];
  static u8 clean_trace[HALF_MAP_SIZE];
  ...
  while (remove_len >= MAX(len_p2 / TRIM_END_STEPS, TRIM_MIN_BYTES)) {
    ...
    while (remove_pos < q->len) {
      ...
      cksum = hash32(trace_bits, HALF_MAP_SIZE, HASH_CONST);
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  if (target_index_plus_one){
    total_branch_tries++;
    if (hits_branch(target_index_plus_one - 1)){
...
bool afl_skip() {
  if (pending_favored) {
    ...
    if ((queue_cur->fuzz_level > 0 || !queue_cur->favored) &&
      UR(100) < SKIP_TO_NEW_PROB) return true;
    ...
    if (queue_cycle > 1 && queue_cur->fuzz_level == 0) {
      ...
      if (UR(100) < SKIP_NFAV_NEW_PROB) return true;

    } else {

      if (UR(100) < SKIP_NFAV_OLD_PROB) return true;
...
static u8 fuzz_one(char** argv) {
 ...
 if (skip_deterministic){
  ...
  if(vanilla_afl) {
    ...
    min_branch_or_block_hits = is_rb_hit_mini(queue_cur->trace_mini, use_block_mutate);
    if(mutation_prior) {
      if(min_branch_or_block_hits == NULL) {
        if(afl_skip()) {
          return 1;
        }
        vanilla_afl = 1; // The normal value for vanilla_afl is 0, so we put it after afl_skip(); If we put it before and skip this seed, next seed will in normal AFL mode
        target_index_plus_one = 0;
      }
    }
    else {
  ...
  if (!vanilla_afl) {
    ...
    int ii;
    for (ii = 0; min_branch_or_block_hits[ii] != 0; ii++){
      target_index_plus_one = min_branch_or_block_hits[ii];
      if (target_index_plus_one){
        int byte_offset = (target_index_plus_one - 1) >> 3;
        int bit_offset = (target_index_plus_one - 1) & 7;

        // skip deterministic if we have fuzzed this min branch or block
        if (queue_cur->fuzzed_branches_or_blocks[byte_offset] & (1 << (bit_offset))){
          // let's try the next one
          continue;
        } else {

          u64 count_from = 0, count_to = HALF_MAP_SIZE >>3;
          if(use_block_mutate) {
            count_from = HALF_MAP_SIZE >> 3;
            count_to = MAP_SIZE >> 3;
          }

          for (int k = count_from; k < count_to; k++){
            //@Ljie@
            //if any branch or block has been fuzzed, then this seed must be fuzzed
            //so we could skip the simple bitflip, why not skip deterministic?
            //might because that stage is used to get the fuzzing_mask 
            if (queue_cur->fuzzed_branches_or_blocks[k] != 0){
          ...
          queue_cur->fuzzed_branches_or_blocks[byte_offset] |= (1 << (bit_offset)); 
          // chose minimum
          break;// we have chosen a target branch, its index is (target_index_plus_one-1)
        }
      } else break; //target_index_plus_one == 0, only has one target rare branch
    }
    // if we got to the end of min_branch_or_block_hits...
    // it's either because we fuzzed all the things in min_branch_or_block_hits -- target_index_plus_one
    // or because there was nothing. If there was nothing, 
    // min_branch_or_block_hits[0] should be 0 
    if (!target_index_plus_one || (min_branch_or_block_hits[ii] == 0)){
      target_index_plus_one = min_branch_or_block_hits[0];
      if (!target_index_plus_one) {
        return 1;
      }
    ...
    ck_free(min_branch_or_block_hits);
    ...
    DEBUG1("which hit branch %i (hit by %u inputs) \n", target_index_plus_one -1, hit_bits[target_index_plus_one -1]);
    //ck_free(rarest_branches);
  ...
  ***************/
  ...
  if (target_index_plus_one && trim_for_branch) {
    ...
    if (trim_len > 0){
      ...
      queue_cur->bitmap_size = count_branch_bytes(trace_bits);
...
re_run: // re-run when running in shadow mode
  ...
  if ((!target_index_plus_one && skip_deterministic) 
    || skip_deterministic_bootstrap 
    || (vanilla_afl && ((!queue_cur->passed_det) && perf_score < (queue_cur->depth * 30 <= HAVOC_MAX_MULT * 100 ? queue_cur->depth * 30 : HAVOC_MAX_MULT * 100)))
    || (vanilla_afl && queue_cur->passed_det))
    goto havoc_stage;
  ...
  if (master_max && (queue_cur->exec_cksum % master_max) != master_id - 1) {
    if (!target_index_plus_one || shadow_mode) goto havoc_stage;
...
#define FLIP_BIT(_ar, _b) do { \
  ...
  /* @RB@ */
  DEBUG1("%swhile bitflipping, %i of %i tries hit branch %i\n", shadow_prefix, successful_branch_tries, total_branch_tries, target_index_plus_one - 1);
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (target_index_plus_one && !shadow_mode && use_branch_mask > 0)
      if (hits_branch(target_index_plus_one - 1)){
  ...
  if (target_index_plus_one && (successful_branch_tries == 0)){
    ...
    blacklist[blacklist_pos++] = target_index_plus_one -1;
    blacklist[blacklist_pos] = -1;
    DEBUG1("adding branch %i to blacklist\n", target_index_plus_one-1);
  }
  ...
  for (stage_cur = 0; stage_cur < (len << 3) - 1; stage_cur++) {
    ...
    if (target_index_plus_one){ //&& use_mask()){
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "afl-fuzz " cBRI VERSION cRST " by <lcamtuf@google.com> Power schedule along with rare branch algorithm\n");
  ...
  while ((opt = getopt(argc, argv, "+bq:rsi:o:f:m:t:T:dnCB:S:M:x:Q:p:vzu")) > 0)
    ...
    switch (opt) {
      ...
      case 'v': /* selection by block coverage */
        if(use_block_select) FATAL("Multiple -v options not supported");
        use_block_select = 1;
        break;

      case 'z': /* mutation by block coverage */
        if(use_block_mutate) FATAL("Multiple -z options not supported");
        use_block_mutate = 1;
        break;

      case 'u': /*Prior the mutation step rather than the selection*/
        if(mutation_prior) FATAL("Multiple -u options not supported");
        mutation_prior = 1;
        break;
